...
function raceOpacityExpression(){return["case",["==",["get","_dominant"],"none"],1,["interpolate",["linear"],["to-number",["get","_dominantShare"],25],25,.22,40,.38,60,.62,80,.82,100,.96]]}
...
function condisRadiusExpression(){return["interpolate",["linear"],["zoom"],8,1.3,10,1.8,12,2.7,14,3.8,16,4.8]}
...
function addRaceLayers(map,data){
 ...
 map.addLayer({id:"race-fill",type:"fill",source:"race",paint:{"fill-color":raceColorExpression(),"fill-opacity":raceOpacityExpression()}});
 map.addLayer({id:"race-line",type:"line",source:"race",paint:{"line-color":"#777777","line-width":.5,"line-opacity":.72}});
}
...
function legendMarkup(type){let extra="";if(type==="disadvantage")extra=`<div class="legend-group"><div class="legend-subtitle">Concentrated Disadvantage - 2020</div>${condisLegendRows()}</div>`;if(type==="boundary")extra=`<div class="legend-group"><div class="legend-subtitle">Boundary annotation</div>${boundaryLegendRows()}</div>`;return `<div class="map-legend"><div class="legend-group"><div class="legend-title">Predominant Racial-Ethnic Group</div>${raceLegendRows()}</div>${extra}<div class="legend-note">Color opacity increases as the mapped predominant group's share increases. Click a block group for exact racial-ethnic shares.</div></div>`}
function figureCard({num,title,caption,key,type,boundary=false}){return `<article class="figure-card"><div class="figure-heading"><div><span class="figure-number">Figure ${num}</span><h3>${title}</h3></div><div class="figure-controls">${boundary?`<div class="draw-tools" data-map-key="${key}"><button type="button" class="draw-mode" data-mode="sharp">Draw sharp</button><button type="button" class="draw-mode" data-mode="fuzzy">Draw fuzzy</button><button type="button" class="secondary small draw-undo">Undo</button><button type="button" class="secondary small draw-clear">Clear</button></div>`:""}<span class="legend-control-slot" data-legend-controls-for="${key}"></span></div></div>${boundary?`<p class="drawing-help">Select a boundary type, click along the boundary, and double-click to finish. Sharp boundaries are solid orange; fuzzy boundaries are black zig-zag lines. Finish your framing before drawing fuzzy boundaries so the zig-zag scale stays consistent.</p>`:""}<div class="map-shell"><div id="map-${key}" class="map" aria-label="${title}"></div></div>${legendMarkup(type)}<p class="caption">${caption}</p></article>`}
function renderSections(){
 ...
 document.querySelectorAll("[data-legend-controls-for]").forEach(slot=>{const key=slot.dataset.legendControlsFor,card=slot.closest(".figure-card"),mapHost=card?.querySelector(".map-shell"),source=card?.querySelector(".map-legend");if(!mapHost||!source)return;window.CC_LEGEND_PREVIEW?.register({key,controlsHost:slot,mapHost,raw:true,rerenderOnScale:true,render:scale=>renderExportLegendPreview(key,scale)})});
...
function createCitywideMap(n1f,n2f){
 const key="citywide",map=new maplibregl.Map({container:`map-${key}`,style:baseStyle(),interactive:true,attributionControl:false,canvasContextAttributes:{preserveDrawingBuffer:true}});
 ...
 map.on("load",()=>{
   ...
   fit(map,bboxThing(cityBoundary),24,11.5);addRacePopup(map,key);
...
function createNeighborhoodMap(key,feature,withDisadvantage,boundaryMode){
 const nbox=bboxThing(feature),view=expandBox(nbox,.72),localRace=filteredPolygons(raceData,view),map=new maplibregl.Map({container:`map-${key}`,style:baseStyle(),interactive:true,attributionControl:false,canvasContextAttributes:{preserveDrawingBuffer:true}});
 map.addControl(new maplibregl.NavigationControl({showCompass:false}),"top-right");addFineZoom(map);
 map.on("load",()=>{
   addRaceLayers(map,localRace);
   if(withDisadvantage){const localCondis=filteredPoints(condisData,view);map.addSource("condis",{type:"geojson",data:localCondis});map.addLayer({id:"condis-points",type:"circle",source:"condis",paint:{"circle-radius":condisRadiusExpression(),"circle-color":condisColorExpression(),"circle-stroke-color":"#555555","circle-stroke-width":["interpolate",["linear"],["zoom"],8,.55,12,.9,16,1.2],"circle-opacity":1}})}
   addNeighborhoodOutline(map,feature,"neighborhood","#111111","#ffffff",3.5);
   if(boundaryMode)initDrawing(key,map);
   fit(map,view,22,14);addRacePopup(map,key);
   map.once("idle",()=>markReady(key));setTimeout(()=>{if(!readyMaps.has(key))markReady(key)},6500);
 });maps[key]=map;
}
function renderMaps(){
...
function setDrawMode(key,mode){const s=drawStates[key],map=maps[key];if(!s||!map)return;if(s.current.length>=2)finishDrawing(key);else{s.current=[];updatePreview(key)}s.mode=s.mode===mode?null:mode;map.getCanvas().style.cursor=s.mode?"crosshair":"";syncDrawButtons(key)}
...
function drawExportLegend(ctx,canvas,type,scale){
 ...
 if(type==="disadvantage"){
   yy+=4*s;ctx.font=`600 ${small}px Arial,sans-serif`;ctx.fillText("Concentrated Disadvantage - 2020",x+pad,yy);yy+=14*s;ctx.font=`${small}px Arial,sans-serif`;
   for(const k of [1,2,3,4,5]){drawDot(ctx,x+pad+4.5*s,yy-3*s,3.8*s,condis[k].color);ctx.fillStyle="#333";ctx.fillText(condis[k].label,x+pad+14*s,yy);yy+=lineH}
 }else if(type==="boundary"){
   yy+=4*s;ctx.font=`600 ${small}px Arial,sans-serif`;ctx.fillText("Boundary annotation",x+pad,yy);yy+=15*s;ctx.strokeStyle=SHARP;ctx.lineWidth=3.5*s;ctx.beginPath();ctx.moveTo(x+pad,yy-4*s);ctx.lineTo(x+pad+29*s,yy-4*s);ctx.stroke();ctx.font=`${small}px Arial,sans-serif`;ctx.fillStyle="#333";ctx.fillText("Sharp boundary",x+pad+38*s,yy);yy+=lineH;drawZig(ctx,x+pad,yy-5*s,29*s);ctx.fillStyle="#333";ctx.fillText("Fuzzy boundary",x+pad+38*s,yy)
 }
 ctx.fillStyle="#666";ctx.font=`${Math.max(6.8*s,6.8)}px Arial,sans-serif`;ctx.fillText("Opacity increases with predominant-group share.",x+pad,y+height-9*s);
 ctx.restore();
}
function renderExportLegendPreview(key,sizeMultiplier=1){const type=legendTypeForKey(key),s=sizeMultiplier*1.25,rows=4+(type==="disadvantage"?6:type==="boundary"?3:0),width=210*s,height=(55+rows*16+(type==="race"?8:18))*s,edge=10*s,canvas=document.createElement("canvas");canvas.width=Math.ceil(width+edge);canvas.height=Math.ceil(height+edge);drawExportLegend(canvas.getContext("2d"),canvas,type,sizeMultiplier);const img=document.createElement("img");img.src=canvas.toDataURL("image/png");img.alt="Export legend preview";img.style.cssText=`display:block;width:${canvas.width}px;height:${canvas.height}px;max-width:none;max-height:none`;return img}
async function captureMap(key){
 const map=maps[key];if(!map)throw new Error("Map is not ready.");if(hasUnfinishedDrawing(key))throw new Error("Finish the active boundary line by double-clicking before exporting.");await waitForIdle(map);map.triggerRepaint();await new Promise(r=>requestAnimationFrame(()=>requestAnimationFrame(r)));const src=map.getCanvas(),canvas=document.createElement("canvas");canvas.width=src.width;canvas.height=src.height;const ctx=canvas.getContext("2d");ctx.drawImage(src,0,0);const scale=Math.max(window.devicePixelRatio||1,1);drawExportLegend(ctx,canvas,legendTypeForKey(key),scale*(window.CC_LEGEND_PREVIEW?.getScale(key)??1));const blob=await new Promise((resolve,reject)=>canvas.toBlob(v=>v?resolve(v):reject(new Error("Could not export map image.")),"image/png"));return{blob,width:canvas.width,height:canvas.height};
}
