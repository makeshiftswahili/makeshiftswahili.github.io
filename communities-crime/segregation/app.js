...
function renderExportLegendPreview(key,sizeMultiplier=1){const type=legendTypeForKey(key),s=sizeMultiplier*1.25,rows=4+(type==="disadvantage"?6:type==="boundary"?3:0),width=210*s,height=(55+rows*16+(type==="race"?8:18))*s,edge=10*s,canvas=document.createElement("canvas");canvas.width=Math.ceil(width+edge);canvas.height=Math.ceil(height+edge);drawExportLegend(canvas.getContext("2d"),canvas,type,sizeMultiplier);canvas.style.width=`${canvas.width}px`;canvas.style.height=`${canvas.height}px`;return canvas}
